...
import { atomRouteAreas, atomRouteDescription, atomRouteWaypoints } from '../../../recoil/atoms';
import { MuiFileInput } from 'mui-file-input'
import { Stack } from '@mui/material';
import { useCSVReader } from 'react-papaparse';
import { calcBearingBetween, calcDistanceBetween, calc_time_to_go, intHoursToStrHoursAndMinutes } from '../../../utils';
...
export default function TableRouteToolBar(props) {
  ...
  const handleClickAddWP = () => {
    ...
    set_route_waypoints((oldRows) => [...oldRows, {
      id: id,
      name: "",
      note: "",
      latitude: 2,
  ...
  const calculateCourse = () => {
    ...
    for (let i = 1; i < new_wps.length; i++) { // Start from 1 because we need to calculate course from previous waypoint
      // Calculate course 
      let course = calcBearingBetween(new_wps[i - 1].latitude, new_wps[i - 1].longitude, new_wps[i].latitude, new_wps[i].longitude)
      course = Math.round(course * 100) / 100 // Round to 2 decimals

      // Calculate distance
      let distance = calcDistanceBetween(new_wps[i - 1].latitude, new_wps[i - 1].longitude, new_wps[i].latitude, new_wps[i].longitude)
  ...
  return (
    <GridToolbarContainer>
      ...
      <Button color='inherit' disabled={true}>Generate XTE ZONE</Button>
      <Button color='inherit' disabled={true}>Generate Available Water ZONE</Button>
      <Button color='inherit' onClick={calculateCourse}>CALC</Button>
